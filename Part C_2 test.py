...
epsilon_3, theta_3 = 0.00029, np.radians(45)  # Strain from gauge 3

# Set up the transformation matrix
A = np.array([
    [np.cos(theta_1) ** 2, np.sin(theta_1) ** 2],
    [np.cos(theta_2) ** 2, np.sin(theta_2) ** 2],
    [np.cos(theta_3) ** 2, np.sin(theta_3) ** 2]
])

# Right-hand side vector with measured strains
b = np.array([epsilon_1, epsilon_2, epsilon_3])

# Solve for epsilon_x and epsilon_y using least squares
epsilon_x_y = np.linalg.lstsq(A, b, rcond=None)[0]
